...
export async function GET(
  ...
  try {
    ...
    // SVG Data (raw QR)
    const qrSvg = await QRCode.toString(assetUrl, {
      type: 'svg',
      margin: 0, // No margin for raw QR used inside card
    ...
    // We scale the QR to fit the 190x190 center area
...
<svg xmlns="http://www.w3.org/2000/svg" width="300" height="340" viewBox="0 0 300 340">
  ...
  <!-- Header Section -->
  <text x="150" y="45" text-anchor="middle" font-family="system-ui, sans-serif" font-size="18" font-weight="800" fill="#0f172a">${asset.name}</text>
  <text x="150" y="68" text-anchor="middle" font-family="JetBrains Mono, monospace" font-size="13" font-weight="600" fill="#64748b" letter-spacing="1">${asset.assetCode || 'NO-CODE'}</text>
  ...
  <text x="150" y="318" text-anchor="middle" font-family="system-ui, sans-serif" font-size="10" font-weight="700" fill="#94a3b8" letter-spacing="2.5">EQUIPMENT TAG</text>
</svg>`
