...
describe('canChangePMGeneratedWOFields', () => {
  it('allows any edit on a non-PM work order', () => {
    ...
    expect(canChangePMGeneratedWOFields(normalWO, {})).toEqual({ allowed: true })
    expect(canChangePMGeneratedWOFields(normalWO, { type: 'PREDICTIVE' })).toEqual({ allowed: true })
  })
  ...
  it('allows leaving the type untouched on a PM work order', () => {
    expect(canChangePMGeneratedWOFields(pmWO, { type: 'PREVENTIVE' })).toEqual({ allowed: true })
  })
})
