...
export default function WorkOrderDetailPane({ woId, onLoadingChange, userRole = 'TECHNICIAN', userId = '' }: Props) {
  ...
  return (
    <div className="p-5 space-y-5 max-w-3xl">
        ...
        <dl className="space-y-3">
          {[
            ...
            { label: 'Assigned to', value: wo.assignedTo?.name ? (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-100 rounded-full text-[10px] font-bold">👤 {wo.assignedTo.name}</span>
            ) : (
      ...
      {wo.notes && (
        <div className="premium-card p-4 border border-slate-200/50 shadow-sm bg-white">
          <h3 className="font-bold text-slate-700 text-xs tracking-tight mb-2">Final Action</h3>
