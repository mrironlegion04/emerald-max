...
/**
 * Work orders generated by a PM schedule (identified by `maintenanceScheduleId`)
 * have fields managed by the schedule itself. The type is fixed to PREVENTIVE
 * and an existing category may be switched between categories but never cleared.
 * Sending an empty category is a harmless no-op when the work order never had
 * one (the edit form always includes the field).
 */
export function canChangePMGeneratedWOFields(
  wo: { maintenanceScheduleId: string | null; woCategoryId: string | null },
  changes: { type?: string; woCategoryId?: string | null }
): { allowed: boolean; reason?: string } {
  if (!wo.maintenanceScheduleId) {
    return { allowed: true }
  }
  if (changes.type !== undefined && changes.type !== 'PREVENTIVE') {
    return { allowed: false, reason: 'Type is managed by the PM schedule and cannot be changed' }
  }
  if (changes.woCategoryId !== undefined && !changes.woCategoryId && wo.woCategoryId) {
    return { allowed: false, reason: 'Category cannot be cleared on a schedule-generated work order' }
  }
  return { allowed: true }
}
